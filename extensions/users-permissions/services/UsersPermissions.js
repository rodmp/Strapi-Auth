...
module.exports = {
  ...
  getActions() {
    ...
    const generateFieldPermissions = (fields) =>
      fields.reduce((acc, field) => {
        acc[field] = { enabled: false, policy: "" };
        return acc;
      }, {});
      ...
      .reduce(
        (acc, key) => {
          Object.keys(strapi.api[key].controllers).forEach((controller) => {
            acc.controllers[controller] = {
              ...generateActions(strapi.api[key].controllers[controller]),
              ...generateFieldPermissions(
                Object.keys(
                  _.get(strapi, `api.${key}.models.${key}.attributes`, {})
                )
              ),
            };
          });
      ...
      Object.keys(_.get(strapi.api[api], "controllers", {})).forEach(
        (controller) => {
          ...
          const fields = Object.keys(
            _.get(strapi, `api.${api}.models.${controller}.attributes`, {})
          ).map(
            (action) => `application.${controller}.${action.toLowerCase()}`
          );
  ...
  async updateRole(roleID, body) {
    ...
    await Promise.all(
      Object.keys(body.permissions || {}).reduce((acc, type) => {
        Object.keys(body.permissions[type].controllers).forEach(
          (controller) => {
